...
`upi_code` is `null` on every record: ANNA-DSB templates are JSON Schemas,
not records — the actual UPI string for an instance is generated by the DSB
live API at registration time and isn't extractable from the cloned repo.
