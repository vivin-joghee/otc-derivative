...
The full Module 4 written analysis is in `src/module4_analysis.md`, structured around four sub-tasks. In brief: the three event-contract trades T026, T027 and T028 all pass Brandes's (2026) economic-function test — each has an identifiable actor with measurable contractual exposure to a political, macroeconomic or regulatory outcome and would benefit from a price-discovery-providing hedging instrument. A proposed UPI schema for the political-outcome variant introduces an `EventContract` asset class with attributes capturing the event description, type, jurisdiction, oracle source and cash-only delivery. The jurisdictional asymmetry between CFTC and MAS / EU treatment of these contracts is regulatory arbitrage in operation: actors with US regulatory access (T026, T028 via Kalshi) benefit, EU residents forced offshore (T027 via Polymarket VPN) and EU supervisors lose. Kalshi's ~13 B USD monthly notional (Brandes 2026; Diercks, Katz & Wright 2026) sits entirely outside any SDR because no `EventContract` asset class exists in ANNA-DSB — a silent miss the current taxonomy cannot help with.
